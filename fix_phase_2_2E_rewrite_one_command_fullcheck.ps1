# fix_phase_2_2E_rewrite_one_command_fullcheck.ps1
# Rewrites phase_2_2E_one_command_fullcheck.ps1 with known-good content (UTF-8 no BOM)
# and performs a parse check. Backs up old file first.
...
$target = Join-Path $projectRoot "phase_2_2E_one_command_fullcheck.ps1"

$stamp = Get-Date -Format "yyyyMMdd_HHmmss"
$backupDir = Join-Path $projectRoot ("backups\manual_edits\PHASE_2_2E_REWRITE_{0}" -f $stamp)
...
if (Test-Path $target) {
  Copy-Item -LiteralPath $target -Destination (Join-Path $backupDir "phase_2_2E_one_command_fullcheck.ps1.BEFORE") -Force
  Ok "Backed up existing target to: $backupDir"
} else {
  Warn "Target did not exist yet. Will create new: $target"
}
...
try {
  ...
  Ok "PHASE 2.2E FULLCHECK COMPLETE — ALL GREEN"
}
finally {
...
Ok "Wrote clean target: $target"

# Parse check MUST pass (no execution)
[ScriptBlock]::Create((Get-Content -Raw $target)) | Out-Null
Ok "Target parse check GREEN."

# Open for visual verification
notepad.exe $target
