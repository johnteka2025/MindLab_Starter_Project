...
const puzzlesRes = await fetchJson<any>(`/puzzles${qp}`);
  ...
  async function solve() {
    ...
    if (isSelectedSolved) {
      setResultMsg("â„¹ï¸ This puzzle is already solved. Pick another one.");
      return;
    }
    ...
    try {
      ...
      if (res?.correct === true) setResultMsg("âœ… Correct!");
      else if (res?.correct === false) setResultMsg("âŒ Incorrect.");
      else setResultMsg("âœ… Submitted.");
    } catch (e: any) {
          ...
          <div style={{ marginTop: "1rem" }}>
            ...
            {isSelectedSolved && (
              <p style={{ marginTop: "0.5rem" }}>
                âœ… Already solved. Select a different puzzle.
              </p>
            )}
