...
function Stop-PortProcess([int]$port) {
  $listenPid = Get-ListeningPidOnPort $port
...
$listenPid = $puzzles[0].id
Write-Host "POST /progress/solve for puzzleId=$listenPid"
$r4 = Http-PostJson $solveUrl @{ puzzleId = $listenPid }
