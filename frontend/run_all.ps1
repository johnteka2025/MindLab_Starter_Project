...
# Local-only specs (NO PROD SPEC HERE)
$localSpecs = @(
...
# Sanity: ensure all spec files exist
$missing = $localSpecs | Where-Object { -not (Test-Path $_) }
if ($missing.Count -gt 0) {
    Write-Host "ERROR: The following local test files were not found:" -ForegroundColor Red
    $missing | ForEach-Object { Write-Host "  - $_" -ForegroundColor Red }
    exit 1
}

# Build Playwright args
$pwArgs = @("playwright", "test") + $localSpecs
if ($TraceOn) {
    $pwArgs += "--trace=on"
}
