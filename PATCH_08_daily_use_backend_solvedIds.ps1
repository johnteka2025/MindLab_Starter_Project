# C:\Projects\MindLab_Starter_Project\PATCH_08_daily_use_backend_solvedIds.ps1
# Goal: DailyChallengeDetailPage.tsx uses backend /progress solvedIds (stable solved state across refresh).
# Golden Rules: absolute paths, backups, sanity checks, fail fast, end at project root.
...
try {
  ...
  Copy-Item $dailyFile ($dailyFile + ".bak_backendSolvedIds_" + $ts) -Force
  Write-Host "Backup created: $($dailyFile).bak_backendSolvedIds_$ts" -ForegroundColor Green

  # Use SINGLE-QUOTED here-string to avoid PowerShell mangling TS template literals
  $content = @'
...
export default function DailyChallengeDetailPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        // Load puzzles + progress in parallel
        const [puzzlesRes] = await Promise.all([
          apiGet<Puzzle[]>("/puzzles"),
          refreshProgress(),
        ]);
  ...
  async function markSolved() {
    ...
    try {
      ...
      // Prefer solvedIds returned by POST, otherwise refresh
...
'@
  ...
  Write-Host "Replaced file: $dailyFile" -ForegroundColor Green
  ...
  Write-Host "PATCH_08 GREEN: Daily page now uses backend solvedIds." -ForegroundColor Green
}
finally {
