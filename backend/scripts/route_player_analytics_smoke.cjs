...
const router = require("../src/routes/playerAnalytics.cjs");
...
async function main() {
  ...
  const server = app.listen(8113, "127.0.0.1");

  try {
    const result = await postJson(8113, "/game/player-analytics", {
      player: "Maya",
      attempts: [
    ...
    console.log("OK ROUTE PLAYER ANALYTICS SMOKE PASSED");
  } finally {
