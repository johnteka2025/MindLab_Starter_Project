...
const router = require("../src/routes/sessionReport.cjs");
...
async function main() {
  ...
  const server = app.listen(8109, "127.0.0.1");

  try {
    const result = await postJson(8109, "/game/session-report", {
      player: "Maya",
      attempts: [
    ...
    assert(result.body.result.overview.snapshot.dashboard.leaderboard.rank === 2, "expected overview rank=2");

    console.log("OK ROUTE SESSION REPORT SMOKE PASSED");
  } finally {
