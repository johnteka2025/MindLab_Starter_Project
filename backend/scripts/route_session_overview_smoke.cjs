...
const router = require("../src/routes/sessionOverview.cjs");
...
async function main() {
  ...
  const server = app.listen(8107, "127.0.0.1");

  try {
    const result = await postJson(8107, "/game/session-overview", {
      player: "Maya",
      attempts: [
    ...
    console.log("OK ROUTE SESSION OVERVIEW SMOKE PASSED");
  } finally {
