...
# Generate Playwright spec for the Daily UI page (expanded gameplay coverage)
...
$specPath = Join-Path $frontendTests "mindlab-daily-ui.spec.ts"
...
test("Daily UI gameplay – open first puzzle and submit wrong answer when supported", async ({ page }) => {
...
Write-Host "Writing Daily UI spec to: $specPath"
$specContent | Out-File -FilePath $specPath -Encoding UTF8 -Force

Write-Host "Daily UI spec written successfully." -ForegroundColor Green
exit 0
