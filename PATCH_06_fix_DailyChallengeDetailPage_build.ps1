# C:\Projects\MindLab_Starter_Project\PATCH_06_fix_DailyChallengeDetailPage_build.ps1
# Fix: Vite build failing due to corrupted template literal in DailyChallengeDetailPage.tsx.
...
try {
  ...
  Copy-Item $dailyFile ($dailyFile + ".bak_fix_" + $ts) -Force
  Write-Host "Backup created: $($dailyFile).bak_fix_$ts" -ForegroundColor Green

  # IMPORTANT: Use single-quoted here-string to prevent PowerShell from mangling backticks/template strings.
  $content = @'
...
'@
  ...
  Write-Host "PATCH_06 GREEN: build succeeded." -ForegroundColor Green
}
finally {
