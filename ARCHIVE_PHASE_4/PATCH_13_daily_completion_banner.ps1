# C:\Projects\MindLab_Starter_Project\PATCH_13_daily_completion_banner.ps1
# Step 3.4B: Add Daily Completion Banner (shows only when solved === total).
...
try {
  ...
  Copy-Item $dailyFile ($dailyFile + ".bak_banner_" + $ts) -Force
  Write-Host "Backup created: $($dailyFile).bak_banner_$ts" -ForegroundColor Green

  # FULL FILE replacement (ASCII-safe text, banner when solved === total)
  $content = @'
...
'@
  ...
  Write-Host "PATCH_13 GREEN: Daily completion banner added." -ForegroundColor Green
}
finally {
